...
new GLTFLoader().load(`${import.meta.env.BASE_URL}models/snow-main.glb`, gltf => {
  let source: THREE.Mesh | undefined;
  gltf.scene.traverse(object => { if (object instanceof THREE.Mesh) source = object; });
  if (!source) throw new Error("Malha do Nevinho não encontrada");
  const rig = createSnowmanGrabRig(source);
  scene.add(rig.mesh);
  ...
  download.addEventListener("click", async () => {
    ...
    const clip = new THREE.AnimationClip("Nevinho_Grab_Experimental", 2.4, rig.bones.flatMap((bone, i) => [
    ...
    try {
      ...
      const link = document.createElement("a"); link.href = url; link.download = "snowman-grab-experimental.glb"; link.click();
